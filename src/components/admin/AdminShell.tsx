...
import { AdminNavItem, AdminTabId, getRoleLabel } from '../../lib/adminPermissions';
        ...
        <div className="flex-1 min-w-0 flex flex-col min-h-screen">
          <header className="sticky top-0 z-40 bg-white/90 dark:bg-slate-900/90 backdrop-blur border-b border-slate-200 dark:border-slate-800 px-4 md:px-6 py-3 flex items-center justify-between gap-3">
            ...
            <div className="flex items-center gap-2 shrink-0">
              <span className="hidden sm:inline-flex items-center gap-1.5 text-[11px] font-bold px-3 py-1.5 rounded-full bg-teal-50 text-teal-700 dark:bg-teal-900/40 dark:text-teal-300">
                <span className="w-1.5 h-1.5 rounded-full bg-teal-500 animate-pulse" />
                MySQL
              </span>
              <span className="text-[11px] font-black px-3 py-1.5 rounded-full bg-slate-900 text-white dark:bg-slate-700">
                {getRoleLabel(role)}
              </span>
            </div>
          </header>
